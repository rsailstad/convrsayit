...
import { AIGeneratedPhrase, PhraseCard, UIPhraseCard } from '../types';

/**
 * Transforms API phrase data to UI component format
 * @param {PhraseCard} phrase - Raw phrase data from API
 * @returns {UIPhraseCard} - Transformed phrase data for UI components
 */
export const transformPhraseForUI = (phrase: PhraseCard): UIPhraseCard => {
  return {
    ...
    category: phrase.category || phrase.activity || 'General',
 ...
 * @param {PhraseCard[]} phrases - Raw phrase data array from API
 * @returns {UIPhraseCard[]} - Array of transformed phrase data for UI components
 */
export const transformPhrasesForUI = (phrases: PhraseCard[]): UIPhraseCard[] => {
 ...
 * @param {AIGeneratedPhrase} phrase - AI-generated phrase data
 * @param {number} index - Index for generating unique ID
 * @returns {UIPhraseCard} - Transformed phrase data for UI components
 */
export const transformAIPhraseForUI = (phrase: AIGeneratedPhrase, index: number): UIPhraseCard => {
  return {
    id: `ai-${index}`,
 ...
 * @param {AIGeneratedPhrase[]} phrases - Raw AI-generated phrase data array
 * @returns {UIPhraseCard[]} - Array of transformed phrase data for UI components
 */
export const transformAIPhrasesForUI = (phrases: AIGeneratedPhrase[]): UIPhraseCard[] => {
  if (!Array.isArray(phrases)) {
    return [];
  }
